...
export default function CalculatorPage() {
  ...
  const handleCalculate = () => {
    if (!canCalculate) {
      setError(
        "Please enter a valid protein amount and select at least one protein source"
      );
      return;
    }
    calculateProtein();
      ...
      <div className="space-y-8">
        ...
        <div className="text-center relative">
          ...
          <p className="mt-4 text-xl text-white max-w-3xl mx-auto font-medium drop-shadow-md">
            Calculate quality-adjusted protein using scientific DIAAS/PDCAAS
            scores for optimal nutrition planning
          </p>
        </div>
        ...
        <Card variant="elevated" className="p-8 animate-fadeIn">
          <div className="space-y-8">
            ...
            {state.error &&
              !state.error.toLowerCase().includes("percentage") &&
              !state.error.toLowerCase().includes("protein source") && (
        ...
        {state.calculationResult && (
          <Card variant="elevated" className="p-8 animate-fadeIn">
            <div className="space-y-8">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="gradient-success p-6 rounded-2xl text-white shadow-xl hover:scale-105 transition-all duration-300">
                  <div className="text-sm font-semibold opacity-90">
                    Quality-Adjusted Protein
                  </div>
                  <div className="text-3xl font-bold mt-2">
                    {formatProteinAmount(
                      state.calculationResult.qualityAdjustedProtein
                    )}
                  </div>
                ...
                <div className="card-modern p-6 rounded-2xl shadow-xl hover:scale-105 transition-all duration-300">
                  ...
                  <div
                    className={`text-3xl font-bold mt-2 ${getPercentageColorClass(
                      state.calculationResult.proteinQualityPercentage
                    )}`}
                  >
                    {formatPercentage(
                      state.calculationResult.proteinQualityPercentage
              ...
              {state.calculationResult.dvDiscrepancy &&
                state.calculationResult.dvDiscrepancy > 0.5 && (
                  <div className="flex items-start gap-4 p-5 bg-orange-50/90 border-2 border-orange-200 rounded-xl backdrop-blur-sm shadow-md animate-fadeIn">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-orange-700 mt-1 font-medium">
                        Stated protein differs from DV% by{" "}
                        {formatProteinAmount(
                          state.calculationResult.dvDiscrepancy
                        )}
                      </p>
        ...
        <Card variant="outlined" className="p-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="p-5 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 hover:scale-105 transition-all duration-300">
              <h4 className="font-bold text-gray-900 mb-3 text-lg">
                DIAAS vs PDCAAS
              </h4>
              <p className="text-sm text-gray-700 leading-relaxed">
                DIAAS is more accurate and can exceed 1.0, while PDCAAS is
                capped at 1.0. We use DIAAS when available.
              </p>
            </div>
            <div className="p-5 rounded-xl bg-gradient-to-br from-green-50 to-blue-50 border border-green-100 hover:scale-105 transition-all duration-300">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">
                Using the DV% from nutrition labels often provides more accurate
                calculations than stated protein amounts.
              </p>
            </div>
          </div>
        </Card>
