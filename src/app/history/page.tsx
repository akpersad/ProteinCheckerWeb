...
export default function HistoryPage() {
    ...
    if (state.selectedCategory !== ProteinCategory.ALL) {
      history = history.filter((calc) => {
        ...
        if (calc.proteinSources && calc.proteinSources.length > 0) {
          return calc.proteinSources.some(
            (ps) => ps.source && ps.source.category === state.selectedCategory
          );
  ...
  const handleExport = () => {
    try {
      ...
      a.href = url;
      a.download = `protein-calculations-${
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Your protein calculations will appear here. Start by using the
            calculator.
          </p>
          <div className="mt-6">
        ...
        {showStatistics && state.statistics && (
          <Card variant="elevated" className="p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                <div className="text-sm text-green-600 font-medium">
                  Avg Quality-Adjusted
                </div>
                <div className="text-2xl font-bold text-green-700 mt-1">
                  {formatProteinAmount(
                    state.statistics.averageQualityAdjustedProtein
                  )}
                </div>
              </div>
            ...
            {state.statistics.mostUsedSources.length > 0 && (
              <div className="mt-6">
                ...
                <div className="space-y-2">
                  {state.statistics.mostUsedSources
                    .slice(0, 5)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-600 mb-6">
              This will permanently delete all calculation history. This action
              cannot be undone.
            </p>
    ...
    if (calculation.proteinSources && calculation.proteinSources.length > 1) {
      ...
      return validSources
        .map(
          (ps) =>
            `${ps.source.name}${
              ps.percentage ? ` (${ps.percentage.toFixed(1)}%)` : ""
            }`
        )
        .join(" + ");
    }
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-3">
              ...
              <div>
                ...
                <div
                  className={`text-sm font-semibold ${getPercentageColorClass(
                    calculation.digestibilityPercentage
                  )}`}
                >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Are you sure you want to delete this calculation for{" "}
              {getProteinSourceDisplay()}?
            </p>
            <div className="flex gap-3 justify-end">
              <Button
