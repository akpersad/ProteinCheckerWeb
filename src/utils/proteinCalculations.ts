 ...
 */
function calculateWeightedScore(sources: ProteinSourceWithPercentage[]): {
  score: number;
 ...
 */
export function calculateDigestibleProtein(
  ...
  const proteinQualityPercentage =
    (qualityAdjustedProtein / statedProtein) * 100;
 ...
 */
export function compareProteinQuality(
  sourceA: ProteinSource,
  sourceB: ProteinSource
): number {
 ...
 */
export function getProteinQualityRating(
...
export function getScoreColor(score: number): string {
  if (score >= 1.0) {
    return "#527a51"; // earthy forest green
  } else if (score >= 0.8) {
