...
export default function MultiProteinSourcePicker({
  ...
  const [validation, setValidation] = useState<ProteinSourceValidation>({
    isValid: true,
    errors: [],
  });
  ...
  useEffect(() => {
    if (proteinSources.length === 0) {
      onSourcesChange([
        { source: null as unknown as ProteinSource, percentage: undefined },
      ]);
    }
  ...
  const validatePercentages = () => {
    ...
    for (const sourceWithPercentage of proteinSources) {
      if (
        sourceWithPercentage.percentage !== undefined &&
        sourceWithPercentage.percentage !== null
      ) {
        hasPercentages = true;
        ...
        if (
          sourceWithPercentage.percentage < 0 ||
          sourceWithPercentage.percentage > 100
        ) {
    ...
    if (hasPercentages && Math.abs(totalPercentage - 100) > 0.01) {
      errors.push(
        `Percentages must add up to 100% (currently ${totalPercentage.toFixed(
          1
        )}%)`
      );
    }
  ...
  const handleSelectSource = (source: ProteinSource) => {
    ...
    if (editingIndex !== null) {
      ...
      if (usePercentages && newSources.filter((s) => s.source).length > 1) {
        const hasPercentages = newSources.some(
          (s) => s.percentage !== undefined
        );
        if (!hasPercentages) {
          const defaultPercentages = getDefaultPercentages(
            newSources.map((s) => s.source).filter(Boolean)
          );
  ...
  const showPercentageInputs =
    usePercentages && proteinSources.filter((s) => s.source).length > 1;
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex gap-2 items-start">
              ...
              <div className="flex-1">
                <button
                  ...
                  className="w-full p-4 text-left input-modern hover:bg-white/95 focus:outline-none focus:ring-4 focus:ring-green-500/20 transition-all duration-200"
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={
                        sourceWithPercentage.source
                          ? "text-gray-900 font-medium"
                          : "text-gray-500 font-medium"
                      }
                    >
                      {sourceWithPercentage.source?.name ||
      ...
      {proteinSources.filter((s) => s.source).length > 1 && (
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${
              usePercentages
      ...
      <ProteinSourcePicker
        ...
        selectedSource={
          editingIndex !== null && editingIndex < proteinSources.length
            ? proteinSources[editingIndex]?.source
            : null
        }
